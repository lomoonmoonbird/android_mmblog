...
import com.moonmoonbird.mmblog.adapter.CardRecycleViewAdapter;
import com.moonmoonbird.mmblog.listeners.EendlessRecylerOnScrollListener;
import com.moonmoonbird.mmblog.model.ThreadList;
import com.moonmoonbird.mmblog.services.APIClient;
import com.moonmoonbird.mmblog.services.ThreadsService;
import com.moonmoonbird.mmblog.wrapper.LoadWrapper;
...
public class HomeActivity extends BaseActivity {
    ...
    private ThreadList goods;
    private ArrayList<ThreadList.Article> dataList = new ArrayList<>();
    ...
    private void initService(){
        service = APIClient.getClient().create(ThreadsService.class);
    }
    ...
    private void loadGoodsList(){
        Call<ThreadList> call = service.getGoodList("adsasds", page);
        call.enqueue(new Callback<ThreadList>() {
            @Override
            public void onResponse(Call<ThreadList> call, Response<ThreadList> response) {
                goods = response.body();
                if (goods.getData().getResult().size() > 0) {
                    dataList.addAll(goods.getData().getResult());
                    loadwrapper.notifyDataSetChanged();
                }
            }

            @Override
            public void onFailure(Call<ThreadList> call, Throwable t) {
